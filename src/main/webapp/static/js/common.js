...
$(function() {
    ...
    YGUnion.Biz = {Common: {_unionKey: ["wid", "cid","bid","qihoo_id","ext","qid"], writeCookie: function () {
...
function initLogin(){
	$.ajax({
		type: "POST",
		url:"/commodity/getIsLoginAndShoppingcart.sc",
		success: function(data){
			if(!data)return;
			data = eval("(" + data + ")");
...
$.fn.jqShare=function(options){
	...
	if (type!=3)
	{
		...
		if(type==4){
			btn= $("<div>").html('<a class="cblue uc_share_a" href="javascript:;">分享</a>');
		}
		...
		if(type!=4){
			...
			btn.css({'position':'relative','z-index':z_index,'margin':'0 auto'});
		}
